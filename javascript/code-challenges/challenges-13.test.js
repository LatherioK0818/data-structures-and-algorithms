...
const sortByChildren = (charArray) => {
  // Sort by the number of children, then alphabetically by house name
  charArray.sort((a, b) => {
    const numChildrenA = a.children.length;
    const numChildrenB = b.children.length;

    // If the number of children is the same, sort alphabetically
    if (numChildrenA === numChildrenB) {
      return a.house.localeCompare(b.house);
    }

    // Otherwise, sort by the number of children
...
const containsW = (str) => {
  // Use a regular expression to check if the string contains 'w' in lowercase
  const pattern = /w/;
...
const isNum = (input) => {
  // Use a regular expression to check if the input contains a number
  const pattern = /\d/;
