...
const longestString = (arr) => {
  if (!arr || arr.length === 0) {
    return -1; // Return -1 for an empty array or invalid input
  }

  return arr.reduce((longestIndex, currentString, currentIndex, array) => {
    return currentString.length > array[longestIndex].length ? currentIndex : longestIndex;
  }, 0);
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const allHappy = (arr) => {
  return arr && arr.every((str) => str.includes(":)"));
};
