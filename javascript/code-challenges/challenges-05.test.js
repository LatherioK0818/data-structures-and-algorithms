...
const checkValues = (obj, value) => {
  // Using Object.values() to get an array of the object values
  const values = Object.values(obj);

  // Checking if the provided value is in the array of values
...
const updateNumbers = (obj) => {
  ...
  // Mapping the array of entries to the desired format
  const result = entries.map(([name, phoneNumber]) => `${name}: ${phoneNumber}`);
...
const getHouses = (arr) => {
  // Using Set to store unique house names
  const houseSet = new Set();

  // Iterating over the characters array and adding house names to the Set
  ...
  // Converting Set to an array and returning the result
...
const hasChildrenValues = (arr, character) => {
  // Finding the character object in the array
  const characterObj = arr.find((char) => char.name === character);

  // Checking if the character has children using Object.values
